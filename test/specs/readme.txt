...
During my tests, the switchFrame command didn't work as expected when I put the iframe ID. I spent almost 1 day searching the issue. 
Finaly, I discovered that I the switchFrame command works only with number but not with iframe ID :( 
To identify the correct iframe ID, in Firefox console mode ,I ran the command: window.frames

The exercise 3 doesn't have X to close modal dialog. then I used WaitforExists command to capture the message that was closed automatially after few senconds
